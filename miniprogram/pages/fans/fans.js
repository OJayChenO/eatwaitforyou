const db = wx.cloud.database()
const _ = db.command
Page({

  /**
   * 页面的初始数据
   */
  data: {
    
  },

  //跳转到用户界面
  toUser(e) {
    let id = e.currentTarget.dataset.id
    // console.log(id)
    wx.navigateTo({
      url: '../user/user?id=' + id,
    })
  },

  //关注用户
  toFollow(e) {
    let uid = e.currentTarget.dataset.uid
    let index = e.currentTarget.dataset.index
    console.log(index, uid)
    // let is_follow = this.readUserData(uid)
    let fansList = this.data.fansList
    let is_follow = fansList[index].is_follow
    // console.log("is_follow", is_follow)
    var that = this
    let loginOK = this.data.loginOK
    if (!loginOK) {
      wx.showToast({
        icon: 'none',
        title: '请先登录',
        success: function() {
          wx.navigateTo({
            url: '/pages/login/login',
          })
        }
      })
    } else {
      wx.showLoading({
        title: '加载中...',
      })
      let my = this.data.myInfo
      let mid = my._id
      wx.cloud.callFunction({
        // 要调用的云函数名称
        name: 'ToFollow',
        // 传递给云函数的event参数
        data: {
          userId: mid,
          follow: uid,
          is_follow: is_follow
        }
      }).then(res => {
        console.log(res)
        //查询数据库
        db.collection('user').doc(mid)
          .get().then(res => {
            // console.log("更新当前用户", res.data)
            let setmy = res.data;
            //更新用户缓存
            wx.setStorageSync('user', setmy)
            this.setData({
              myInfo: setmy,
            })
            that.readUserData(index)
          })
        wx.hideLoading()
        wx.showToast({
          title: '操作成功',
        })
      }).catch(err => {
        console.log(err)
      })
    }

  },


  //读取用户数据
  readUserData(index) {
    // console.log('读取用户数据')
    let my = this.data.myInfo
    let mid = my._id
    let users = this.data.fansList
    let uid = users[index]._id
    let follow = my.follow
    // 判断是否关注
    let a = follow.indexOf(uid);
    let setFollow = "fansList[" + index + "].is_follow"
    if (a < 0) {
      console.log("未关注")
      this.setData({
        [setFollow]: false
      })
    } else {
      console.log("已关注")
      this.setData({
        [setFollow]: true
      })
    }




  },

  /**
   * 生命周期函数--监听页面加载
   */
  onLoad: function(options) {
    wx.showLoading({
      title: '页面加载中...',
    })
    let that = this
    //获取用户缓存
    let user = wx.getStorageSync('user')
    if (user && user.name) {
      this.setData({
        loginOK: true,
        myInfo: user
      })
    }
    let myInfo = this.data.myInfo
    let fans = this.data.myInfo.fans
    db.collection('user').where({
      _id: _.in(fans)
    }).get().then(res => {
      // console.log(res.data)
      this.setData({
        fansList: res.data
      })
      let fansList = this.data.fansList
      for (let i = 0; i < fansList.length; i++) {
        that.readUserData(i)
      }
    }).catch(err => {
      console.log(err);
    }).finally(wx.hideLoading())

  },

  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  onReady: function() {

  },

  /**
   * 生命周期函数--监听页面显示
   */
  onShow: function() {

  },

  /**
   * 生命周期函数--监听页面隐藏
   */
  onHide: function() {

  },

  /**
   * 生命周期函数--监听页面卸载
   */
  onUnload: function() {

  },

  /**
   * 页面相关事件处理函数--监听用户下拉动作
   */
  onPullDownRefresh: function() {

  },

  /**
   * 页面上拉触底事件的处理函数
   */
  onReachBottom: function() {

  },

  /**
   * 用户点击右上角分享
   */
  onShareAppMessage: function() {

  }
})