...
Page({
  ...
  readUserData(index) {
    ...
    let my = this.data.myInfo
    let mid = my._id
    let users = this.data.follows
    let uid = users[index]._id
    let follow = my.follow
    let setSelf = "follows[" + index + "].is_self"
    // 判断是否自己
    if (uid == mid) {
      console.log("自己")
      this.setData({
      ...
      let setFollow = "follows[" + index + "].is_follow"
      if (a < 0) {
        console.log("未关注")
        this.setData({
          [setFollow]: false
  ...
  onLoad: function(options) {
    ...
    let parseFollows = JSON.parse(options.follows)
    // console.log("parseFollows", parseFollows)
